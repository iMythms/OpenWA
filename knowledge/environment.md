...
- SYA local integration state on 2026-08-14: session `sya-local` (`60549a4b-1610-4379-92ec-6af1c84de6f3`) is linked to the SYA WhatsApp profile, `ready`, and running through `whatsapp-web.js`. Operator key `SYA local integration` is restricted to that session and installed only in the neighboring project's ignored local environment. An explicit outbound integration message was persisted with `delivered` status.
